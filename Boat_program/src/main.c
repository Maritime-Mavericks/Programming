	...
	while(1){ // empty infinite loop
		...
		if(startRescueTransmission == 1){
			if(boatCoordinateReceived == 1){
				...
				if((fabs(destLatitude - latitude) <= 0.00005) && (fabs(destLongitude - longitude) <= 0.00005)){
				...
				float destHeading = getDestHeading(latitude, longitude, destLatitude, destLongitude) + biasAngle;

				// Find direction to rotate
				int turnRightOrLeft = 0; // 0 is right, left is 1float currentLatitude, float currentLongitude, float currentDestLatitude, float currentDestLongitude
				float deltaAngle = 0;
